...
def check_bullet_alien_collisions(ai_settings,screen,stats,sb,ship,aliens,bullets):			
	...
	if len(aliens)==0:
		#Если флот уничтожен, начинается новый уровень
		bullets.empty()
...
def create_fleet(ai_settings, screen, ship, aliens):
	...
	#создание пришельца и вычисления их кол-ва в ряду
	#интервал между пришельцами равет одной ширине пришельца
	alien=Alien(ai_settings,screen)
...
def get_number_rows(ai_settings,ship_height, alien_height):
	"""определяем количство рядов, перемещающихся по экрану"""
...
def check_fleet_edges(ai_settings, aliens):
	"""реагиреут на достижение пришельцем края экрана"""
	for alien in aliens.sprites():
		if alien.check_edges():
			chenge_fleet_direction(ai_settings,aliens)
			break
			
def chenge_fleet_direction(ai_settings,aliens):
...
def ship_hit(ai_settings,stats,screen,sb,ship,aliens,bullets):
	 ...
	 if stats.ship_left>0:
		 ...
		 stats.ship_left-=1
		 #Обновление игровой инфы
		 sb.prep_ship()
 		 #Очистка списков
		 aliens.empty()
		 bullets.empty()
		 #Создани нового флота и размещение корабля в центре.
		 create_fleet(ai_settings,screen,ship,aliens)
		 ship.center_ship()
		 #Пауза.
		 sleep(0.5)
